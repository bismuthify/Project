...
\end{figure}
Figure \ref{fig:detplotsC} shows the differene between equal coupling and nearest neighbor coupling. It takes longer for solutions to become coherent when using nearest neighbor coupling.
\begin{figure}[h]
...
\end{figure}
 Figure \ref{fig:detplotsR0} shows the affect $R_0$ has on the period of oscillations and nature of the cycles.
\begin{figure}[h]
    \caption{$m=0.2$}
...
A stochastic version of this model was simulated exactly using the Gillespie algorithm with the same parameters and similar initial conditions used for the deterministic solution. The population size per patch was 3000. Equal coupling and $R_0=17$ and $m=0.5$ were used. See Figure \ref{fig:Gillplot}.
\begin{figure}[h]
...
\end{figure}
For larger population sizes, approximate stochastic simulations (using the \Rlogo adaptivetau package) were used. See Figure \ref{fig:adapplot} for a simulation with population of 100000.
\begin{figure}[h]
...
\end{figure}
<<bifurcation,fig.height=4,echo=FALSE,eval=FALSE>>=
...
## plot bifurcation
#tikz("bifurcation.tex",standAlone=TRUE,width=6,height=6)
plot(bfd$R0,bfd$I, xlab="$\\R_0$",ylab="Prevalence (I)",pch=20, col="black")
#dev.off()
#tools::texi2dvi('bifurcation.tex',pdf=T)
@
...
<<periodplot,fig.height=4,echo=FALSE,eval=FALSE>>=
