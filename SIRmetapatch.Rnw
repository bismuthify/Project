...
percent <- 0.4
population <- 3000
S0 <- as.integer(population*seq(SEE*(1-percent), SEE*(1+percent), SEE*2*percent/(n-1)))
I0 <- as.integer(population*seq(IEE*(1+percent), IEE*(1-percent), -IEE*2*percent/(n-1)))
R0 <- population-S0-I0
...
<<SIRmeta.adaptivetau,dev="tikz">>=
  library("adaptivetau")
  #For reprodubility set the seed
  set.seed(45)
  n <- 10 #number of patches
  
  #TO DO: This should be changed so that it works with different n values and is less ugly
...
for (i in 1:n) {
  ...
  #Measure of coherence
  coherence <- lapply(1:length(simResults[,1]), function(tims) sqrt(sum((simResults[tims,(12:21)]-mean(simResults[tims,(12:21)])*rep(1,n))^2)) )
  #Plot coherence 
