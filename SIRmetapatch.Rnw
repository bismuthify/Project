...
<<SIRmeta.adaptivetau,dev="tikz",eval=FALSE>>=
  ...
  coherence <- sapply(1:length(simResults[,1]), 
                      function(tims) sd(simResults[tims,(12:21)])/mean(simResults[tims,(12:21)]) )
...
## draw box for plot
  par(mar = c(5,5,2,5))
  plot(0,0,xlim=c(0,tmax/365),ylim=c(0,800),xlab="Time (years)", 
       ylab="Prevalence (I)")
...
<<>>=
 ...
 gridsizem <- 4
 nsample <- 1
 mlist <- seq(0.001, 1, (1-0.001)/(gridsizem-1))
 R_0list <- seq(2,25,(25-2)/(gridsizeR_0-1))
...
for (R_0 in R_0list){
  ...
  for (m in mlist){
    ...
    for (sampler in 1:nsample){
      ...
      coherence[sampler] <- mean(coherenceRun[length(coherenceRun)-(50:0)])
    }
    #Take mean of all coherence for all runs with these parameters
     avgCoherence[index1,index2]<- mean(coherence)
     index1<-index1+1
  }
  index2<-index2+1
}
 ...
 yminval<-min(avgCoherence)
 ymaxval<-max(avgCoherence)
 par(mfrow=c(gridsizem/2,2))
 par(cex = 0.6)
par(mar = c(0, 0, 0, 0), oma = c(4, 4, 0.5, 0.5))
par(tcl = -0.25)
par(mgp = c(2, 0.6, 0))
 for(ind in 1:gridsizem){
   
   #legend("bottomleft",legend=paste0("m=",mlist[ind]),cex=0.5,bty="n",text.col=ind)
   if (ind ==3){
          plot(R_0list, avgCoherence[ind,],type='l',col=ind, ylim=c(yminval,ymaxval))}
         #axis(1, col = "grey40", col.axis = "grey20")}
   if (ind ==1){
          plot(R_0list, avgCoherence[ind,],type='l',col=ind, ylim=c(yminval,ymaxval),xaxt='n')}
         #axis(1, col = "grey40", col.axis = "grey20")}
     if (ind ==2){
          plot(R_0list, avgCoherence[ind,],type='l',col=ind, ylim=c(yminval,ymaxval),xaxt='n',yaxt='n')}
   if (ind ==4){
          plot(R_0list, avgCoherence[ind,],type='l',col=ind, ylim=c(yminval,ymaxval),yaxt='n')}
         #axis(2, col = "grey40", col.axis = "grey20")}
     box(col = "grey60")
     mtext(paste0("m=",mlist[ind]), side = 3, line = -2, adj = 0.1, cex = 0.6, col = "grey40")
 }
