...
draw.soln <- function(n,ic=c(x=rep(1,10),y=rep(0,10)),tmax=1,
  ...
  legend("topright",legend=labels,col=1:n,lty=1)
  text(x=8000,y=0.0005, labels="$\\| I - \\langle I \\rangle  e \\|$")
}
...
SIRmeta.Gillespie <- function(tmax,ic=c(S0,I0,R0),parms=c(R_0=2,gamma=0.25,mu=4e-5,alpha=0.1,m=0.2,EC=TRUE)){
  ...
  while(times[index] < tmax){
    ...
    if(any(S<0) | any(I<0) | any(R<0)){
      ...
      print("interval")
      print(interval)
      print(sum(rateslisted[1:(interval+1)]))
      print(sum(rateslisted[1:(interval)]))
      print("interval diff")
      print(rateslisted[interval+1])
      print("interval rate")
      if(colInd==0) colInd<-6
      print(rates[rowInd,colInd])
      break
    }
    #print(times[index])
    index <- index+1
    ...
    print(times)
  return(c(t= t(times), S =S,I= I, R=R))
}
tmax <- 1
n <- 10
##Initial conditions
