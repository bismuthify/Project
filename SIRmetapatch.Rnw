...
<<>>=
 ...
 gridsizem <- 80
 nsample <- 5
 mlist <- seq(0, 0.99, (0.99-0)/(gridsizem-1))
...
write.table(Adaptau3D,file="Adaptau3DNN.csv")

pdf(file="images/Adaptau3DNN.pdf")
#3D plot
nbcol <- 100
...
bfd$I <- (1/max(bfd$I))*bfd$I
 submlistInd <- c(30,50,70,gridsizem) #indices of m values that will be plotted on 4 panel plot
 par(mfrow=c(2,2))
 par(cex = 0.6)
