...
#                      function(tims) sqrt(sum((soln[tims,paste0('I',1:n)]-mean(soln[tims,paste0('I',1:n)])*rep(1,n))^2)) )
  ...
  #Plot coherence 
  par(new = T)
  plot(times,coherence,type='l',axes=F,xlab=NA, ylab=NA,ylim=c(0,1),lwd=2)
 ...
 # legend("bottomright", legend="Relative Standard Deviation",bg="white",col=1,lwd=2)
}
...
#leave room on right margin for Coherence label
  par(mar = c(5,5,2,5))
plot(0,0,xlim=c(0,tmax),ylim=c(0,0.004),type="n",xlab="Time (years)",
...
write.table(run,file="adaptauECR017m0.01.csv")
pdf(file="images/adaptauECR017m0.01.pdf")
...
Adaptau3D <- cbind.data.frame(R_0list,mlist,avgCoherence)
write.table(Adaptau3D,file="Adaptau3DEC.csv")

pdf(file="images/Adaptau3DEC.pdf")
#3D plot
nbcol <- 100
...
zcol  <- cut(t(avgCoherence), nbcol)
 ...
 pdf(file="images/FourPanelEC.pdf")
...
bfd$I <- (1/max(bfd$I))*bfd$I
 submlistInd <- c(1,30,60,gridsizem) #indices of m values that will be plotted on 4 panel plot
 par(mfrow=c(2,2))
 par(cex = 0.6)
