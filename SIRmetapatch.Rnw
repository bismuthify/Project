...
#Parameters
R_0 <- 25   ##basic reproduction rate (if the system had no forcing)
...
write.table(run,file="adaptauECR025m0.02s.csv")
pdf(file="images/adaptauECR025m0.02s.pdf")
...
#Solutions should ocsillate around IEE so ylim should go to around twice that so solns are in the middle of plot
  plot(0,0,xlim=c(0,tmax),ylim=c(0,population*IEE*3),xlab="Time (years)", 
       ylab="Prevalence (I)")
